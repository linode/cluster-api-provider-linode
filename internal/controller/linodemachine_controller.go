...
func (r *LinodeMachineReconciler) reconcile(ctx context.Context, logger logr.Logger, machineScope *scope.MachineScope) (res ctrl.Result, err error) {
	...
	cond := machineScope.LinodeMachine.GetCondition(ConditionPreflightBootstrapDataSecretReady)
	if cond != nil && cond.Status == metav1.ConditionTrue && machineScope.Machine.Spec.Bootstrap.DataSecretName == nil {
...
func (r *LinodeMachineReconciler) reconcileCreate(
	...
	if machineScope.LinodeMachine.Spec.FirewallRef != nil {
		cond := machineScope.LinodeMachine.GetCondition(ConditionPreflightLinodeFirewallReady)
		if cond != nil && cond.Status == metav1.ConditionTrue && machineScope.LinodeMachine.Spec.ProviderID == nil {
	...
	if vpcRef := getVPCRefFromScope(machineScope); vpcRef != nil {
		cond := machineScope.LinodeMachine.GetCondition(ConditionPreflightLinodeVPCReady)
		if cond != nil && cond.Status == metav1.ConditionTrue && machineScope.LinodeMachine.Spec.ProviderID == nil {
	...
	cond := machineScope.LinodeMachine.GetCondition(ConditionPreflightMetadataSupportConfigured)
	if cond != nil && cond.Status == metav1.ConditionTrue && machineScope.LinodeMachine.Spec.ProviderID == nil {
	...
	cond = machineScope.LinodeMachine.GetCondition(ConditionPreflightCreated)
	if cond != nil && cond.Status == metav1.ConditionTrue && machineScope.LinodeMachine.Spec.ProviderID == nil {
	...
	cond = machineScope.LinodeMachine.GetCondition(ConditionPreflightRootDiskResizing)
	...
	cond = machineScope.LinodeMachine.GetCondition(ConditionPreflightRootDiskResized)
	if cond != nil && cond.Status == metav1.ConditionTrue {
	...
	cond = machineScope.LinodeMachine.GetCondition(ConditionPreflightConfigured)
	if cond != nil && cond.Status == metav1.ConditionTrue {
